...
DGHashmap<string,COLORREF> cols;
...
void initcols(void){
	...
	addcolname("fuchsia",getRGB(255,0,255));
	addcolname("green",getRGB(0,128,0));
	addcolname("gray",getRGB(128,128,128));
	addcolname("grey",getRGB(190,190,190));
	addcolname("lime",getRGB(0,255,0));
	addcolname("maroon",getRGB(128,0,0));
	addcolname("navy",getRGB(0,0,128));
	addcolname("orange",getRGB(255,165,0));
	addcolname("olive",getRGB(128,128,0));
	addcolname("purple",getRGB(128,0,255));
	addcolname("pink",getRGB(255,192,203));
	addcolname("red",getRGB(255,0,0));
	addcolname("silver",getRGB(192,192,192));
	...
	addcolname("zima",getRGB(55,190,237));
...
void polyline(int len,DPOINTS pnts){
	DGPPPOLY *tmp=new DGPPPOLY;
	tmp->len=len;
	tmp->pnts=new DPOINT[len+3];
	for(int i=0;i<len;++i){
...
void vppolyl(int len,va_list vl){
	DPOINT *p=new DPOINT[len+3];
	for(int i=0;i<len;++i){
		p[i]=va_arg(vl,DPOINT);
	}
...
void vpolyl(int len,va_list vl){
	DPOINT *p=new DPOINT[len+3];
	for(int i=0;i<len;++i){
		int x=va_arg(vl,int);
		int y=va_arg(vl,int);
...
void parallelogram(DPOINT p1,DPOINT p2,DPOINT p3){
	int dx=p2.x-p1.x,dy=p2.y-p1.y;
	DPOINT p4=DPOINT(p3.x+dx,p3.y+dy);
	ppoly(4,p1,p2,p4,p3);
}
void parallelogram(int x1,int y1,int x2,int y2,int x3,int y3){
	parallelogram(DPOINT(x1,y1),DPOINT(x2,y2),DPOINT(x3,y3));
}
void para(DPOINT p1,DPOINT p2,DPOINT p3){
	parallelogram(p1,p2,p3);
}
void para(int x1,int y1,int x2,int y2,int x3,int y3){
...
DCOLOR getcolor(const string &colname){
	string _="";
	int r,g,b;
	if(colname[0]=='#'){
...
DCOLOR makecolor(const string &colname){
...
void settransparentfill(){
	brcol=getRGB(0,0,0);
	DeleteObject(g_hbr);
	g_hbr=(HBRUSH)GetStockObject(DGUI_TRANSBRUSH);
	SelectObject(g_hdc,g_hbr);
}
